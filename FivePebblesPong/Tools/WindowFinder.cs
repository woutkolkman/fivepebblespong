...
namespace FivePebblesPong
{
    ...
    public class WindowFinder
    {
        ...
        {
            FivePebblesPong.ME.Logger_p.LogInfo("GetWindowLayout, Searching for window \"" + windowName + "\"");
            ...
            bool callback(int handle) {
                FivePebblesPong.ME.Logger_p.LogInfo("GetWindowLayout, Found window \"" + windowName + "\"");
                GetWindowRect(handle, ref rect);
                return false; //stop searching for windows
            }

            if (!task.Wait(TimeSpan.FromSeconds(2))) //timeout
                FivePebblesPong.ME.Logger_p.LogError("GetWindowLayout, Window \"" + windowName + "\" not found");
            return rect;
        }
